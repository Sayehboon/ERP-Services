    ...
    public async Task<IEnumerable<JournalEntryDto>> Handle(GetAllJournalEntriesQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.SearchTerm))
        {
            ...
            query = query.Where(je => 
                ...
                (je.Account != null && je.Account.Name != null && je.Account.Name.ToLower().Contains(searchTerm)) ||
                (je.Account != null && je.Account.Code != null && je.Account.Code.ToLower().Contains(searchTerm)) ||
