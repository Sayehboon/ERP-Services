...
public class JournalEntriesController : BaseController
{
	...
	/// <summary>
	/// ایجاد سند حسابداری جدید (بر اساس JournalVoucher)
	/// </summary>
	[HttpPost]
	...
	public async Task<ActionResult> PostJournalEntry(Guid id)
	{
		try
		{
			var result = await _mediator.Send(new PostJournalVoucherCommand(id));
			if (!result) return BadRequest("اعتبارسنجی سند نامعتبر است");
